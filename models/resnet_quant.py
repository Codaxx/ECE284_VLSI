...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, float=False):
        ...
        self.inplanes = 8                                                                                    #changed
        self.conv1 = nn.Conv2d(3, 8, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(8)
        self.relu = nn.ReLU(inplace=True)            
        self.layer1 = self._make_layer(block, 8, layers[0], float=float)                                 #changed    
        self.layer2 = self._make_layer(block, 16, layers[1], stride=2, float=float)                       #changed
        self.layer3 = self._make_layer(block, 48, layers[2], stride=2, float=float)                       #changed
        self.avgpool = nn.AvgPool2d(8, stride=1)
        self.fc = nn.Linear(48 * block.expansion, num_classes)                                            #changed
